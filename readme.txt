-------------------------------------------------------------------------------
this Readme is the same as the original readme in 
https://sites.google.com/site/findcommunities/ for updated version but i add this little 
part below
-------------------------------------------------------------------------------
#Part added
-----------------------------------------------------------------------------
file,folders added to louvain project,are main.cc, parser.py,graph_gen.cpp cavemans/,bins/, tree/

in graph_gen.cpp you will find a graph caveman generator and a 
benchmark performance according to the size of clicks or number of clicks

after generating the graphs benchmarking them, then storing the result 
in Result.txt, each line exept the first is formated as: clicksize numberofclicks time
the first line tell the number of links between clicks
commands to execute
-make : generate louvain execs
-make bin : generate graph generator and benchmark exec named tbin
-./tbin : will genreate graphs in the folders cavemans/, bins/
and execute louvain algorithm, benchmark time execution
and store result in Result.txt
-python parser.py read Result.txt and plot a graph
showing time execution in function of numbers of click and size clicks
